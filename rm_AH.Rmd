...
```{r}
ggplot(filtered_sales_lm, aes(log(square_footage),log(amount), color=group)) +
...
leaflet()  %>% 
    addProviderTiles("Stamen.TonerHybrid")  %>% 
    addPolygons(color ="grey", opacity=1, weight= 0.5) %>%
...
```{r}
filter <- filtered_sales_lm[ , c("amount", "square_footage", "age", "group", "ownerdate", "tract")]
filter
```
...
```{r}
#write.csv(filter, "filter.csv", row.names = FALSE)
```
